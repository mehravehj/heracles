...
class SequentialMemory(Memory):
    ...
    def append(self, observation, action, reward, terminal, training=True):  # (s_t, a_t, r_t, done) are all in one layer
        super(SequentialMemory, self).append(observation, action, reward, terminal, training=training)

        # This needs to be understood as follows: in `observation`, take `action`, obtain `reward`
        # and weather the next state is `terminal` or not.
        if training:
            self.observations.append(observation)  # s{t}, so in observation: [s{0}, s{1}, s{2}, ..., s{current_timestep}, ...]
            self.actions.append(action)  # a{t}, so in actions: [a{0}, a{1}, a{2}, ..., a{current_timestep}, ...]
            self.rewards.append(reward)  # r{t}, so in rewards: [r{0}, r{1}, r{2}, ..., r{current_timestep}, ...]
